...
#
# Runs Claude Code headlessly in repeated iterations. Each iteration picks
# the next incomplete story from prd.json, implements it, commits, and
# updates progress. Exits when all stories pass or max iterations reached.
...
if [ ! -f "$PRD_FILE" ]; then
  ...
  echo "Generate one first using the kickoff phase."
  exit 1
fi
