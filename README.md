...
- [Librarian (Cross-Task Learning)](#librarian-cross-task-learning)
...
| `OPENAI_API_KEY`        | —                         | OpenAI API key for generating embeddings (`text-embedding-3-small`). Required when `LIBRARIAN_ENABLED=true`. Get one at [platform.openai.com](https://platform.openai.com).   |
...
1. **Before each phase** — the Librarian searches for semantically relevant learnings from past tasks and injects them into the agent's prompt (or writes `learnings.md` for the Ralph loop)
2. **After each phase** — the agent's output is analyzed by a separate Claude call that extracts reusable learnings (patterns, gotchas, architecture decisions)
3. **Deduplication** — each new learning is compared against existing ones via embedding similarity. A Librarian decision agent decides whether to file it as new, merge it with an existing learning, replace an outdated one, or skip it
